...
const groupCount = ref(classInfo.value?.groupCount || 4)
...
const getGroupMembers = (group: Group) => {
    ...
    return group.members.map((member) => {
        const currentStudent = classInfo.value!.students.find((s) => s.id === member.id)
...
const persistGroups = debounce(() => {
    if (classInfo.value) {
        classesStore.updateGroups(classInfo.value.id, localGroups.value)
    }
}, 500)
...
const randomAssignGroups = () => {
    ...
    if (confirm('這將重新分配所有學生，確定要繼續嗎？')) {
        ...
        const presentStudents = classInfo.value.students.filter((s) => s.isPresent)
...
const addGroupScore = (groupId: string, score: number) => {
    if (!classInfo.value || !classInfo.value.groupingActive) return
    classesStore.addScoreToGroup(classInfo.value.id, groupId, score)
}
...
const startGrouping = () => {
    if (classInfo.value) {
        classesStore.startClassGrouping(classInfo.value.id)
    }
}
...
const confirmEndGrouping = () => {
    if (classInfo.value) {
        classesStore.endClassGrouping(classInfo.value.id)
    }
...
const exportActivityReport = () => {
    ...
    rows.push([`班級: ${classInfo.value.name}`])
...
watchEffect(() => {
    if (classInfo.value) {
        localGroups.value = JSON.parse(JSON.stringify(classInfo.value.groups || []))
        groupCount.value = classInfo.value.groupCount || 4
    }
})
</script>
