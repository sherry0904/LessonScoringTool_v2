    ...
    <div class="modal modal-open">
        <div class="modal-box w-11/12 max-w-md">
            ...
            <div v-if="students.length > 0">
                <!-- Winner Display -->
                <div v-if="ui.pickerWinner && !ui.isPicking" class="text-center my-8 animate-fade-in">
                    <p class="text-lg text-base-content/70">抽中的是...</p>
                    <p class="text-6xl font-bold my-4 text-primary">{{ ui.pickerWinner.name }}</p>
                    <p class="text-2xl font-mono text-base-content/50">座號: {{ ui.pickerWinner.id }}</p>
                </div>

                <!-- Picker Animation -->
                <div v-else class="h-48 my-8 flex items-center justify-center overflow-hidden relative">
                    ...
                    <div v-else class="text-2xl text-base-content/40">
                        準備開始抽籤...
                    </div>
                    <div class="absolute top-1/2 left-0 right-0 h-16 -translate-y-1/2 border-y-2 border-primary/50 rounded-lg"></div>
                </div>
...
const shuffledNames = computed(() => {
    ...
    if (ui.pickerWinner) {
        ...
        if (lastWinnerIndex !== -1) {
            ...
            [list[lastWinnerIndex], list[secondToLastIndex]] = [list[secondToLastIndex], list[lastWinnerIndex]];
        }
    }
    return list
})
...
.animate-picker-scroll {
    /* This animation is illustrative. A real implementation would dynamically calculate offset */
    animation: scroll 3s cubic-bezier(0.25, 0.1, 0.25, 1) forwards;
